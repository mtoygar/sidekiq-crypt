...
class Sidekiq::CryptTest < Sidekiq::Crypt::TestCase
  ...
  def test_raises_error_if_key_store_and_current_version_does_not_match
    assert_raised_error("current_key_version can't be found in key_store") do
      configure_sidekiq_crypt(
        options: {
          current_key_version: 'V2', key_store: { 'V1' => Base64.strict_encode64('123') }
        }
      )
    end
  end
  ...
  def test_raises_error_if_current_key_is_not_valid_for_encryption
    assert_raised_error('current key is not valid for encryption') do
      configure_sidekiq_crypt(
        options: {
          current_key_version: 'V1', key_store: { 'V1' => Base64.strict_encode64('123') }
        }
      )
    end
  end
  ...
  def test_stringfy_key_store_keys_when_configure_used
    configure_sidekiq_crypt(options: {
                              ...
                              key_store: {
                                V1: Base64.strict_encode64('ThisPasswordIsReallyHardToGuess!'),
                                V2: Base64.strict_encode64('246')
                              }
                            })
    ...
    assert_equal(
      Base64.strict_encode64('ThisPasswordIsReallyHardToGuess!'), config.key_store['V1']
    )
    assert_equal(Base64.strict_encode64('246'), config.key_store['V2'])
  end
